...
class Dense_model(nn.Module):
    def __init__(self,drop_rate):
        ...
        self.fc4 = torch.nn.Linear(1024, 20)
...
def main():
    ...
    rand_p = 0.75  # data augmentation random proportion
    lr = 1e-3
    lr_decay = 0.5
    ...
    test2_files = [{"input": in_img}
                   for in_img in test2_img]

    stanford_files = [{"input": in_img}
                      for in_img in stanford_img]
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        v_loss = loss_func(v_loss_os,v_loss_pfs,v_loss_age,v_loss_label, v_loss_img)
                    ...
                    for test_data in test_loader:
                        ...
                        s_loss = loss_func(s_loss_os,s_loss_pfs,s_loss_age,s_loss_label,s_loss_img)
    ...
    if model_run:
        ...
        # # test model ON EGFR + COHORTS
        # model.load_state_dict(torch.load(BEST_MODEL_NAME + "_PFS.pth"))
        # _, pfs_risk, bn_feature = model_run_gpu(model, test2_files, val_transforms)
        #
        # model.load_state_dict(torch.load(BEST_MODEL_NAME + "_OS.pth"))
        # os_risk, _, _ = model_run_gpu(model, test2_files, val_transforms)
        ...
        # bn_feature.to_csv(BEST_MODEL_NAME + "_feature_independent.csv")


        # test model ON STANFORD COHORTS
        model.load_state_dict(torch.load(BEST_MODEL_NAME + "_PFS.pth"))
        _, pfs_risk, bn_feature = model_run_gpu(model, stanford_files, val_transforms)

        model.load_state_dict(torch.load(BEST_MODEL_NAME + "_OS.pth"))
        os_risk, _, _ = model_run_gpu(model, stanford_files, val_transforms)
        pred_save = torch.cat((os_risk, pfs_risk), 1)
        pred_save = pred_save.cpu().numpy()
        pred_save = pd.DataFrame(pred_save)
        pred_save.to_csv(BEST_MODEL_NAME + "_hugo.csv")

        bn_feature = bn_feature.cpu().numpy()
        bn_feature = pd.DataFrame(bn_feature)
        bn_feature.to_csv(BEST_MODEL_NAME + "_feature_hugo.csv")
